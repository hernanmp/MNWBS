  ...
  for(ind_p in 1:length(p_grid)  )
  {
    ...
    for(iter in 1:NMC)
    {
      ...
      for(t in 1:T)
      {
        if(t <v[1] ||  t > v[2])
        {
          y[t,] = mvrnorm(n = 1, mu0, sqrt(1.25)*Sigma0)
          z[t,] = mvrnorm(n = 1, mu0, sqrt(1.25)* Sigma0)
        }
        ...
        if(t >=v[1] &&  t < v[2])
        {
          ...
          if(coin == 1)
          {
            y[t,] = mvrnorm(n = 1, mu1, Sigma0)
            z[t,] = mvrnorm(n = 1, mu1, Sigma0)
          }
          if(coin == 0)
          {
            y[t,] = mvrnorm(n = 1, mu2, Sigma0)
            z[t,] = mvrnorm(n = 1, mu2, Sigma0)
          }
